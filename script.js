...
// SLICE: slice(start, end before)  // Does not mutate
...
// SPLICE: splice(start, delete count)  // Does mutate
...
movements.forEach(function (mov, ind, arr) {
  if (mov > 0) {
    console.log(`Movement ${ind + 1}  You deposited ${mov}`);
  } else {
    console.log(`Movement ${ind + 1} You withdrew ${Math.abs(mov)}`);
  }
});
// 0: function(200)
// 1: function(450)
...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  // const dogs = dogs3.concat(dogsKate)
  console.log(dogs);
...
//       const result = acc + age / arr.length;
...
// const arr1 = ar1.concat(ar2, ar3, ar4)
// console.log(arr1);
...
// Emprty arrays + fill method: array.fill(value to be filled, start index, stop index)
const x = new Array(7);
console.log(x);
// console.log(x.map(() => 5));
...
// 4
// this is an nice title -> This Is a Nice Title
...
// 3.
const sums = accounts // can also be destructured
  .flatMap(acc => acc.movements)
  .reduce(
    (sum, cur) => {
...
const convertTitleCase = function (title) {
  ...
  const result = title
    .toLowerCase()
    .split(' ')
    .map(item => (exceptions.includes(item) ? item : capitzalize(item)))
    .join(' ');
  return capitzalize(result);
};
console.log(convertTitleCase('this is a nice title'));
console.log(convertTitleCase('this is a LONG title but not too long'));
console.log(convertTitleCase('and here is another title with an EXAMPLE'));
...
const findSarahDog = function () {
  const sarahDog = dogs.find(dog => dog.owners.find(item => item === 'Sarah'));
...
// // actual solution    note: I don't have a "recFood"
// // const dogSarah = dogs.find(dog => dog.owners.includes('Sarah' || 'sarah'));
...
//   dogs.map(cur => {
//     if (cur.curFood > cur.recommendedFood) ownersEatTooMuch.push(cur.owners);
//     if (cur.curFood < cur.recommendedFood) ownersEatTooLittle.push(cur.owners);
//   });
...
// 5
const equal = dogs.map(dog => dog.curFood === dog.recommendedFood);
console.log(equal);
...
// actual solution
const okArr = dogs.filter(checkEatingOkay);
console.log(okArr);
